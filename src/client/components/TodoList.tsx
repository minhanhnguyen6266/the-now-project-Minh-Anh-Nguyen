...
export const TodoList = ({ status }: { status: Status }) => {
  ...
  // delete
  const { mutate: deleteOne } = api.todo.delete.useMutation({
    onSuccess: () => {
      apiContext.todo.getAll.refetch()
    },
  })
  ...
  // status call api
  if (isLoading) {
    return <div className="mt-2 flex justify-center">Đang tải...</div>
  }
        ...
        <li key={todo.id}>
          <div className="flex items-center justify-between rounded-12 border border-gray-200 px-4 py-3 shadow-sm">
            <div className="flex justify-center">
              <Checkbox.Root
                onCheckedChange={() => handleChange(todo)}
            ...
            <div>
              <XMarkIcon
                className="rounded-lg h-8 w-8 cursor-pointer gap-2 p-1"
